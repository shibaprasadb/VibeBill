...
Precedence rule: **Option A**. Each event is priced by the card with the greatest `effectiveFrom` on or before the event's UTC date, where the loaded table card is treated as effective from its `asOf` and each `prices-history.json` row from its `effectiveFrom`. So a valid user-refreshed table governs usage on or after its `asOf`, older usage resolves against bundled history, and — importantly — a history row dated *after* `asOf` still takes effect for later events rather than being shadowed by the stale table card. When no dated card applies (an event predating every source), the loaded table card is the fallback. History rows are order-independent (selection does not assume the JSON is pre-sorted). User-supplied `prices-history.json` or `prices-history.csv` files in the config directory are ignored; invalid or ambiguous files produce warnings and the bundled history remains authoritative.
